...
# URL base
BASE_URL="https://archive.org/download/redump-nec-pc-engine-cd-turbografx-cd-2021-06-06"
...
download_file() {
    ...
    while IFS= read -r line && [ $i -le $index ]; do
        if [ $i -eq $index ]; then
            ...
            wget -P "../Roms/PCECD/" "$BASE_URL$line"

            # Replace invalid chars from the name
            file_name=$(echo "$line" | sed -e 's/%20/ /g' -e 's/%28/(/g' -e 's/%29/)/g' -e 's/%2C/,/g' -e 's/%26/\&/g' -e 's/%27/'"'"'/g' -e 's/%21/!/g' -e 's/%25/%/g')

            # Rename file with the new file name
            mv "../Roms/SEGACD/$line" "../Roms/PCECD/$file_name"
            echo "Download complete: ../Roms/PCECD/$file_name"
            break
        fi
        i=$((i + 1))
    done < file_list.txt
